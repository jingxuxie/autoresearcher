...
| 0004 | completed | needs_human | needs_human | `metrics.environment_audit.reward_audit.non_success_reward`=0.0<br>`metrics.environment_audit.reward_audit.success_reward`=1.0<br>`metrics.exact_dp.non_tie_policy_informative`=True<br>`metrics.exact_dp.raw_normalized_policy_preserved`=True<br>`metrics.exact_dp.rows.0.gamma`=0.95 |
...
- `0004`: Protected file drift remains unresolved in loop metadata despite current git status not showing tracked protected-path modifications.
- `0004`: The 0004 result omits the required drift status even though the plan explicitly required it.
- `0004`: The behavior stream is oracle-guided by exact normalized Q with epsilon, so the result tests matched-stream estimator quality rather than fully online learner-induced exploration.
