#!/usr/bin/env python3
"""Experiment 0005: successor-distance lambda/equivalence audit."""
...
def equivalence_report(
    ...
    return {
        "max_abs_q_diff": max_abs_q_diff(mdp, candidate_q, reference_q),
        "max_abs_value_diff": max_abs_value_diff(mdp, candidate_q, reference_q),
        "action_diff_rate": action_diff_rate(mdp, candidate_q, reference_q),
        "heldout_mse_delta": candidate_metrics["heldout_long_horizon_value_mse"] - reference_metrics["heldout_long_horizon_value_mse"],
        "policy_regret_delta": candidate_metrics["policy_regret"] - reference_metrics["policy_regret"],
        "q_overestimation_delta": candidate_metrics["q_overestimation_error"] - reference_metrics["q_overestimation_error"],
        "is_equivalent": (
            max_abs_q_diff(mdp, candidate_q, reference_q) <= EQUIVALENCE_TOL
            and max_abs_value_diff(mdp, candidate_q, reference_q) <= EQUIVALENCE_TOL
            and action_diff_rate(mdp, candidate_q, reference_q) <= EQUIVALENCE_TOL
        ),
        "equivalence_tolerance": EQUIVALENCE_TOL,
    }
...
def run_scenario(mdp: MDP, trajectories: List[Dict[str, Any]]) -> Tuple[Dict[str, Any], Dict[str, Any], Dict[str, Any], Dict[str, Any]]:
    ...
    successor_meta: Dict[str, Any] = {}
    for lambda_tr in LAMBDA_SWEEP:
        method_name = lambda_method_name(lambda_tr)
        methods[method_name], successor_meta[method_name] = train_successor_distance_trl_log(mdp, trajectories, lambda_tr)
    metrics = {
    ...
    for lambda_tr in LAMBDA_SWEEP:
        method_name = lambda_method_name(lambda_tr)
        equivalence[method_name] = {
            "lambda_tr": lambda_tr,
            "versus_trl_log": equivalence_report(
                mdp,
                methods[method_name],
                methods["trl_log"],
                metrics["methods"][method_name],
                metrics["methods"]["trl_log"],
            ),
            "versus_successor_calibration_only": equivalence_report(
                mdp,
                methods[method_name],
                methods["successor_calibration_only"],
                metrics["methods"][method_name],
                metrics["methods"]["successor_calibration_only"],
            ),
        }
    metrics["equivalence_diagnostics"] = equivalence
    ...
    for lambda_tr in LAMBDA_SWEEP:
        method_name = lambda_method_name(lambda_tr)
        q_values = methods[method_name]
        successor_tables[method_name] = {
            "lambda_tr": lambda_tr,
            "q_score": serialize_q(mdp, q_values),
            "q_distance": serialize_distance_q(mdp, q_values),
            "value_score": values_from_q(mdp, q_values),
            "value_distance": serialize_value_distances(values_from_q(mdp, q_values)),
        }
            ...
            rows.append(
                {
                    ...
                    "method": method_name,
                    "lambda_tr": scenario.get("successor_meta", {}).get(method_name, {}).get("lambda_tr"),
...
def main() -> int:
    ...
    all_equivalence = {
        scenario_id: scenario["equivalence_diagnostics"]
        for scenario_id, scenario in all_metrics["scenarios"].items()
    }
    main_scenario_ids = [
        scenario_id
        for scenario_id, scenario in all_metrics["scenarios"].items()
        if scenario["scenario_role"] != "stress_biased"
    ]
    ...
    for lambda_tr in LAMBDA_SWEEP:
        method_name = lambda_method_name(lambda_tr)
        succ_rows = [row for row in main_rows if row["method"] == method_name]
        succ_mean_heldout = sum(row["heldout_long_horizon_value_mse"] for row in succ_rows) / len(succ_rows)
        improves = succ_mean_heldout < cal_mean_heldout - EQUIVALENCE_TOL
        equivalent_all_main = all(
            all_equivalence[scenario_id][method_name]["versus_trl_log"]["is_equivalent"]
            for scenario_id in main_scenario_ids
        )
        non_equivalent_any_main = not equivalent_all_main
        matched_no_regret_increase = (
            safe_matched[method_name]["policy_regret"] <= safe_matched["successor_calibration_only"]["policy_regret"] + EQUIVALENCE_TOL
            and risk_matched[method_name]["policy_regret"] <= risk_matched["successor_calibration_only"]["policy_regret"] + EQUIVALENCE_TOL
        )
        matched_no_overestimate_increase = (
            safe_matched[method_name]["q_overestimation_error"] <= safe_matched["successor_calibration_only"]["q_overestimation_error"] + EQUIVALENCE_TOL
            and risk_matched[method_name]["q_overestimation_error"] <= risk_matched["successor_calibration_only"]["q_overestimation_error"] + EQUIVALENCE_TOL
        )
        risk_optimal_selects_risky = risk_matched[method_name]["eval_greedy_action"] == "risky"
        positive_successor_evidence = all(
            [
                improves,
                non_equivalent_any_main,
                matched_no_regret_increase,
                matched_no_overestimate_increase,
                risk_optimal_selects_risky,
            ]
        )
        lambda_summaries[method_name] = {
            "lambda_tr": lambda_tr,
            "main_mean_heldout_mse": succ_mean_heldout,
            "improves_vs_calibration_only": improves,
            "equivalent_to_trl_log_all_main": equivalent_all_main,
            "non_equivalent_to_trl_log_any_main": non_equivalent_any_main,
            "matched_no_policy_regret_increase_vs_calibration_only": matched_no_regret_increase,
            "matched_no_q_overestimation_increase_vs_calibration_only": matched_no_overestimate_increase,
            "risk_optimal_selects_risky": risk_optimal_selects_risky,
            "positive_successor_evidence": positive_successor_evidence,
        }

    any_positive_successor_evidence = any(item["positive_successor_evidence"] for item in lambda_summaries.values())
    improving_lambdas = [item for item in lambda_summaries.values() if item["improves_vs_calibration_only"]]
    all_improving_lambdas_equivalent_to_trl_log = bool(improving_lambdas) and all(
        item["equivalent_to_trl_log_all_main"] for item in improving_lambdas
    )
    negative_equivalence_evidence = not any_positive_successor_evidence and all_improving_lambdas_equivalent_to_trl_log
    audit_completed = chain_raw_valid and chain_log_valid and bool(lambda_summaries)
    all_metrics["aggregate"] = {
        "successor_calibration_only_main_mean_heldout_mse": cal_mean_heldout,
        "lambda_summaries": lambda_summaries,
        "any_positive_successor_evidence": any_positive_successor_evidence,
        "negative_equivalence_evidence": negative_equivalence_evidence,
        "all_improving_lambdas_equivalent_to_trl_log": all_improving_lambdas_equivalent_to_trl_log,
    }
    all_metrics["success_checks"] = {
        "chain_raw_exact": chain_raw_valid,
        "chain_trl_log_exact": chain_log_valid,
        "lambda_sweep_completed": bool(lambda_summaries),
        "positive_successor_evidence": any_positive_successor_evidence,
        "negative_equivalence_evidence": negative_equivalence_evidence,
        "audit_completed": audit_completed,
    }

    artifact_paths = [
        f"research/{PROJECT}/artifacts/{EXPERIMENT_ID}/run_lambda_equivalence_audit.py",
        f"research/{PROJECT}/artifacts/{EXPERIMENT_ID}/raw_metrics.json",
        f"research/{PROJECT}/artifacts/{EXPERIMENT_ID}/metrics.csv",
        f"research/{PROJECT}/artifacts/{EXPERIMENT_ID}/lambda_sweep.json",
    ...
    write_json(
        artifact_dir / "lambda_sweep.json",
        {
            "lambda_sweep": list(LAMBDA_SWEEP),
            "equivalence_tolerance": EQUIVALENCE_TOL,
            "summaries": lambda_summaries,
        },
    )
    write_json(artifact_dir / "equivalence_diagnostics.json", all_equivalence)
    ...
    if any_positive_successor_evidence:
        interpretation = (
            "At least one successor-distance lambda improved held-out MSE versus calibration-only while remaining non-equivalent to trl_log and preserving matched risky-regret and overestimation checks."
        )
        ...
        known_failures = [
            "No lambda produced positive non-equivalent successor-distance evidence versus trl_log within the predeclared tolerance."
        ] if not negative_equivalence_evidence else []
    result = {
        "experiment_id": EXPERIMENT_ID,
        "status": "completed" if audit_completed else "failed",
        "claim_tested": (
            "A successor-distance lambda sweep can reveal whether successor_distance_trl_log has a distinct effect beyond trl_log while retaining calibration and lower held-out error than calibration-only."
        ),
...
- Lambda sweep: `{list(LAMBDA_SWEEP)}`
- Equivalence tolerance: `{EQUIVALENCE_TOL}`
...
"""
    for row in rows:
        summary += (
            f"| {row['scenario_id']} | {row['method']} | {row['lambda_tr']} | {row['heldout_long_horizon_value_mse']:.12f} | "
...
- Positive successor-distance evidence: `{any_positive_successor_evidence}`
- Negative equivalence evidence: `{negative_equivalence_evidence}`
- All improving lambdas equivalent to trl_log: `{all_improving_lambdas_equivalent_to_trl_log}`
...
- `research/{PROJECT}/artifacts/{EXPERIMENT_ID}/run_lambda_equivalence_audit.py`
- `research/{PROJECT}/artifacts/{EXPERIMENT_ID}/raw_metrics.json`
- `research/{PROJECT}/artifacts/{EXPERIMENT_ID}/metrics.csv`
- `research/{PROJECT}/artifacts/{EXPERIMENT_ID}/lambda_sweep.json`
